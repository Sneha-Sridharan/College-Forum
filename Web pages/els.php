...
<!-- your mega menu starts here! -->
								...
								<li class="nav-item active"><a class="nav-link dropdown-toggle" href="#" id="clubsDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Clubs</a>
								...
								<!-- finally, using flex to create your layout -->
								 <div class="d-md-flex align-items-start justify-content-start">
								  ...
								  <div>
								   ...
								   <a class="dropdown-item active" href="els.php">English Literary Society</a>
								  ...
								  <div>
								   ...
								   <a class="dropdown-item" href="#">Women Development Cell</a>
									...
									echo "<li class='nav-item'><a class='nav-link' href='Reg1.php'>Register</a></li>";
									}
									else{
									echo "<li class='nav-item'><a class='nav-link' href='login4.php'>Register</a></li>";
									}
									?>
								<?php
									if(isset($_SESSION['username'])){
...
<h1 style="font-family:'Zilla slab',serif;font-size:40px;text-align:center;margin-top:200px;color:white">ENGLISH LITERARY SOCIETY</h1><br>
<div class="column">
<h2>DESCRIPTION</h2>
<hr>
<p>
...
$sql="SELECT clubinfo FROM clubs WHERE clubid='ELTS'";
$result=$conn->query($sql);
$row = $result->fetch_assoc();
echo $row["clubinfo"];
?>
</p>
<img src="els.jpg" style="margin-left:100px;" alt="els" width="30%" height="50%"><br><br>
<h2>RECENT EVENTS</h2>
<hr>
<?php
$sql = "SELECT poster FROM events WHERE clubid='ELTS' AND date_event>=CURDATE() ORDER BY date_event";
...
<pre>
<?php
$sql="SELECT faculty_advisor FROM clubs WHERE clubid='ELTS'";
$result=$conn->query($sql);
$row = $result->fetch_assoc();
...
<pre>
<?php
$sql = "SELECT secretary1,secretary2 FROM clubs WHERE clubid='ELTS'";
$result = $conn->query($sql);
$row = $result->fetch_assoc();
echo $row["secretary1"];
?>
...
<pre>
<?php
$sql="SELECT contact FROM clubs WHERE clubid='ELTS'";
$result=$conn->query($sql);
$row = $result->fetch_assoc();
echo $row["contact"];
?>
...
<?php
$sql="SELECT recr_link,message FROM events WHERE clubid='ELTS'";
$result=$conn->query($sql);
$row = $result->fetch_assoc();
