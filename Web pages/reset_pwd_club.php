<?php
	session_start();
	if(isset($_SESSION['reset_pwd_club_error2']))
	{	
		echo '<script type="text/javascript">alert("The new password and confirm new password did not match! Please enter correctly");</script>';
		unset($_SESSION['reset_pwd_club_error2']);
	}
	if(isset($_SESSION['reset_pwd_club_error1']))
	{	
		echo '<script type="text/javascript">alert("Old password is incorrect!");</script>';
		unset($_SESSION['reset_pwd_club_error1']);
	}
?>
<html>
	<head>
...
<h1 style="text-align:center;font-size:40px;">   RESET PASSWORD   </h1>
	<form class="reg-form" method="POST" action="reset_pwd_club_test.php" enctype="multipart/form-data">
