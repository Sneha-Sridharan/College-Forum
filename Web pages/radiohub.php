...
<!-- your mega menu starts here! -->
								...
								<!-- finally, using flex to create your layout -->
								 <div class="d-md-flex align-items-start justify-content-start">
								  ...
								  <div>
								   <a class="dropdown-item active" href="radiohub.php">Radio Hub</a>
...
<h1 style="font-family:'Zilla slab',serif;font-size:40px;text-align:center;margin-top:200px;color:white">RADIO HUB</h1><br>
<div class="column">
<h2>DESCRIPTION</h2>
<hr>
<p>
...
$sql="SELECT clubinfo FROM clubs WHERE clubid='RDHB'";
$result=$conn->query($sql);
$row = $result->fetch_assoc();
echo $row["clubinfo"];
?>
</p>
<img src="radh.jpg" style="margin-left:100px;" alt="radio hub" width="30%" height="50%"><br><br>
<h2>RECENT EVENTS</h2>
<hr>
<?php
$sql = "SELECT poster FROM events WHERE clubid='RDHB' AND date_event>=CURDATE() ORDER BY date_event";
$result = mysqli_query($conn, $sql);
  if(mysqli_num_rows($result)>0) {
	while($row = $result->fetch_assoc()) {
	echo '<img src="data:image/jpg;base64,'.base64_encode($row['poster']).'" style="float:left;margin-right:50px;width:30%;height:90%"/>';
  }	
  }
  else {
...
<pre>
<?php
$sql="SELECT faculty_advisor FROM clubs WHERE clubid='RDHB'";
$result=$conn->query($sql);
$row = $result->fetch_assoc();
...
<pre>
<?php
$sql = "SELECT secretary1,secretary2 FROM clubs WHERE clubid='RDHB'";
$result = $conn->query($sql);
$row = $result->fetch_assoc();
echo $row["secretary1"];
?>
...
<pre>
<?php
$sql="SELECT contact FROM clubs WHERE clubid='RDHB'";
$result=$conn->query($sql);
$row = $result->fetch_assoc();
echo $row["contact"];
?>
...
<?php
$sql="SELECT recr_link,message FROM events WHERE clubid='RDHB'";
$result=$conn->query($sql);
$row = $result->fetch_assoc();
